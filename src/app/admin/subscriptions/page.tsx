...
export default function AdminSubscriptionsPage() {
  ...
  const handleAppFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const detail =
        err?.response?.data?.detail || "Failed to register application. Please try again.";
  ...
  const handlePackageFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const detail =
        err?.response?.data?.detail || "Failed to save package. Please try again.";
      setToast({ message: detail, type: "error" });
    } finally {
  ...
  const handleConfirmDeletePackage = async () => {
    ...
    try {
      ...
      const detail =
        err?.response?.data?.detail || "Failed to delete package. Please try again.";
      setToast({ message: detail, type: "error" });
    } finally {
    ...
    try {
      ...
      setToast({
        message:
          ...
          "Failed to fetch admin data from subscription service (Port 8003).",
        type: "error",
      });
    } finally {
      setLoading(false);
              ...
              <div>
                ...
                {modalMode === "create" && lockedAppId && (
                  <p className="text-[11px] text-dim mt-1 flex items-center gap-1">
                    <Info className="w-3 h-3 text-accent-dark shrink-0" />
                    App ID locked for package creation on platform "{packageFormData.app_id}".
                  </p>
                )}
              </div>
